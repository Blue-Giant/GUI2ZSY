...
class MY_GUI(object):
    def __init__(self, init_window):
        ...
        self.init_window.title("文本处理工具_v1.2")      # 窗口名
        self.init_window.geometry('400x500+10+10')     # 290 160为窗口大小，+10 +10 定义窗口弹出时的默认展示位置
    ...
    def set_init_window(self):
        ...
        self.name2DNN_Text = TK.Entry(self.init_window, width=20, textvariable=str2DNN)  # 网络模型选择框
        self.name2DNN_Text.grid(row=0, column=2)

        str2Hidden = TK.StringVar()
        str2Hidden.set('(5,10,20,5)')   # 是一个列表或者元组
        self.hiddens_Text = TK.Entry(self.init_window, width=20, textvariable=str2Hidden)    # 隐藏层神经单元列表输入框
        self.hiddens_Text.grid(row=1, column=2)

        str2act_In = TK.StringVar()
        str2act_In.set('ReLU')          # 默认 ReLU
        self.actIn_Text = TK.Entry(self.init_window, width=20, textvariable=str2act_In)  # 输入层的激活函数
        self.actIn_Text.grid(row=2, column=2)

        str2act_Hidden = TK.StringVar()
        str2act_Hidden.set('ReLU')      # 默认 ReLU
        self.actHidden_Text = TK.Entry(self.init_window, width=20, textvariable=str2act_Hidden)  # 隐藏层的激活函数
        self.actHidden_Text.grid(row=3, column=2)

        str2act_Out = TK.StringVar()
        str2act_Out.set('linear')       # 默认 linear
        self.actOut_Text = TK.Entry(self.init_window, width=20, textvariable=str2act_Out)  # 输出层的激活函数
        self.actOut_Text.grid(row=4, column=2)

        int2in_dim = TK.IntVar()
        int2in_dim.set(2)              # 默认 2
        self.dim2in_Text = TK.Entry(self.init_window, width=20, textvariable=int2in_dim)  # 输入维度
        self.dim2in_Text.grid(row=5, column=2)

        int2out_dim = TK.IntVar()
        int2out_dim.set(1)             # 默认 1
        self.dim2out_Text = TK.Entry(self.init_window, width=20, textvariable=int2out_dim)  # 输出维度
        ...
        self.button2Start = TK.Button(self.init_window, text='开始训练', command=self.get_print_log_parameters)
    ...
    def get_print_log_parameters(self):
        ...
        # TK.messagebox.showinfo(self.init_window, text="Successfully init and parameters", width=20)
