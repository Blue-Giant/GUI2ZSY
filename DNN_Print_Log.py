...
def dictionary_out2file(R_dic, log_fileout, actName2normal=None, actName2scale=None):
    log_string('Type for problem: %s\n' % (R_dic['PDE_type']), log_fileout)
    log_string('Name for problem: %s\n' % (R_dic['equa_name']), log_fileout)
    ...
    if R_dic['model2Normal'] == 'Fourier_DNN':
        ...
        log_string('The scale to fourier layer: %s\n' % (R_dic['sFourier']), log_fileout)
    if R_dic['model2Scale'] == 'Fourier_DNN':
    ...
    log_string('Loss function: %s\n'% str(R_dic['loss_type']), log_fileout)

    if R_dic['opt2loss_orthogonal'] == 'with_orthogonal':
        log_string('With the orthogonality for coarse and fine. \n', log_fileout)
        log_string(str(R_dic['if_contrib2scale_part']) + ' scale for the orthogonality of coarse and fine. \n', log_fileout)
        log_string('The loss of product for coarse and fine:' + str(R_dic['model2orthogonal']) + '\n', log_fileout)
    else:
    ...
    log_string('Batch-size 2 testing: %s\n' % str(R_dic['batch_size2test']), log_fileout)

    log_string('using loss2bd or not: %s\n' % str(R_dic['opt2loss_bd']), log_fileout)
...
def print_and_log_train_one_epoch(i_epoch, run_time, learn_rate, penalty_powu, pwb, loss_it_tmp,
